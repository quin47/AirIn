...
func (c *capture) Start() (<-chan []int16, error) {
	...
	if exe == "" {
		return nil, fmt.Errorf("no audio capture tool found (tried: arecord, parec); install alsa-utils or pulseaudio-utils")
	}
	...
	chunkSize := sampleRate * channels / 10 // 100ms chunks
